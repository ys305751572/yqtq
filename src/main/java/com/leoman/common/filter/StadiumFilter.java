...
public class StadiumFilter implements Filter {
    ...
    public void doFilter(ServletRequest request,
        ...
        StadiumUser stadiumUser = (StadiumUser)httpRequest.getSession().getAttribute(Constant.SESSION_MEMBER_GLOBLE);
        if(stadiumUser!=null){
            chain.doFilter(request, response);
            return;
        }
        ...
        if (xRequested != null && xRequested.indexOf("XMLHttpRequest") != -1) {
            ...
            httpResponse.sendRedirect(contextPath+"/stadium/login");
        }
    }
